...
function scheduledArrivalToMinutes(scheduledArrival) {
  if (!scheduledArrival) return null;
  
  const [hours, minutes, seconds] = scheduledArrival.split(':').map(Number);
  const now = new Date();
  const nowSeconds = now.getHours() * 3600 + now.getMinutes() * 60 + now.getSeconds();
  const arrivalSeconds = hours * 3600 + minutes * 60 + seconds;
  
  const diff = Math.round((arrivalSeconds - nowSeconds) / 60);
  return diff < 0 ? null : diff;
}

export function adaptStationToBusCard(station) {
  const nextArrival = station?.next_arrivals?.[0];
  const minutes = scheduledArrivalToMinutes(nextArrival?.scheduled_arrival);
  return {
    id: station.stop_id,
